...
test("getCanonicalQuery", async (t) => {
	...
	await t.test(
		"should return an empty string for requests with no querystring",
		async (t) => {
	...
	await t.test("should URI-encode keys and values", async (t) => {
		...
		t.same(
			canonicalQuery,
			"%F0%9F%90%8E=%F0%9F%A6%84&%F0%9F%92%A9=%E2%98%83%EF%B8%8F",
		);
	});
	await t.test(
		"should omit the x-amz-signature parameter, regardless of case",
		async (t) => {
			const { requestData } = t.context;
			requestData.query = {
				"x-amz-signature": "foo",
	...
	await t.test(
		"should serialize arrays using an alphabetic sort",
		async (t) => {
			const { requestData } = t.context;
	...
	await t.test("should URI-encode members of query param arrays", async (t) => {
		...
		t.same(
			canonicalQuery,
			"%F0%9F%90%8E=%F0%9F%92%A9&%F0%9F%90%8E=%F0%9F%A6%84",
		);
	});
	await t.test(
		"should sort URI-encode members of query param arrays",
		async (t) => {
			const { requestData } = t.context;
	...
	await t.test(
		"should omit non-string, non-array values from the serialized query",
		async (t) => {
			const { requestData } = t.context;
			requestData.query = { foo: "bar", baz: new Uint8Array(0) };
			const canonicalQuery = getCanonicalQuery(requestData);
